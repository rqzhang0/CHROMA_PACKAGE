...
namespace Chroma
{
  ...
  {
    MomShellQuarkSourceConstEnv::Params tmp(xml, path);
    param = tmp;
  }
  ...
  namespace MomShellQuarkSourceConstEnv
  {
	...
	return new SourceConst<LatticeStaggeredPropagator>(Params(xml_in, path));
      ...
      //! Name to be used
      const std::string name("MOM_SHELL_SOURCE");
    }
    ...
    void Params::writeXML(XMLWriter& xml, const std::string& path) const
    {
      ...
      write(xml, "SourceType", "SHELL_SOURCE");
	...
	//
	multi1d<LatticeColorMatrix> u_smr_tmp = u,u_smr(Nd);
	{
	  ...
	  (*linkSmearing)(u_smr_tmp);
	  //u_smr=u_smr_tmp;
	}
		
	//
	//Multiply the phase of momentum smearing
	...
	for(int mu=0; mu<Nd; mu++)
	{
		...
		if(mu<Nd-1){	
		u_smr[mu]=u_smr_tmp[mu]*cmplx(cos(K),-sin(K));
		}
		else if(mu==Nd-1){
		u_smr[mu]=u_smr_tmp[mu];
		}
