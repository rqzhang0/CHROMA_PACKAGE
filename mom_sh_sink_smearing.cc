...
#include "mom_sh_sink_smearing.h"
...
namespace Chroma
{
  ...
  {
    MomShellQuarkSinkSmearingEnv::Params tmp(xml, path);
    param = tmp;
  }
  ...
  namespace MomShellQuarkSinkSmearingEnv
  {
	...
	return new SinkSmear<LatticeFermion>(Params(xml_in, path), u);
      ...
      //! Name to be used
      const std::string name("MOM_SHELL_SINK");
    ...
    void Params::writeXML(XMLWriter& xml, const std::string& path) const
    {
      ...
      write(xml, "SinkType", MomShellQuarkSinkSmearingEnv::getName());
    ...
    SinkSmear<LatticePropagator>::operator()(LatticePropagator& quark_sink) const
    {
      ...
      try
      {
	multi1d<LatticeColorMatrix> u_smr_mom(Nd);
	   for(int mu=0; mu<Nd; mu++)
           {
	                Real K= 0.5*twopi/Layout::lattSize()[mu];
	                     QDPIO::cout<< mu << ":"<< cos(K)<<"+"<<-sin(K)<<"I"<<std::endl;
	                     if(mu<Nd-1){    
		                u_smr_mom[mu]=u_smr[mu]*cmplx(cos(K),-sin(K));
	                   }
                     else if(mu==Nd-1){
                u_smr_mom[mu]=u_smr[mu];
                }
	...
	if (params.quark_smear_firstP)
	{
	  ...
	  // Sink smear quark
	  //
	  (*quarkSmearing)(quark_sink, u_smr_mom);

	  //
	  // Displace the sink
	  //
	  (*quarkDisplacement)(quark_sink, u_smr_mom, PLUS);
	}
	else
	{
	  //
